...
export default function WhatsAppButton({label="WhatsApp", size="md", variant="solid"}:Props){
  const sizes = {
  ...
  const styles = {
    solid: "bg-whatsapp text-white hover:opacity-90",
    ghost: "border border-whatsapp text-whatsapp hover:bg-green-50",
    link: "underline underline-offset-4 text-whatsapp hover:opacity-80"
  }[variant];

  const handleClick = () => {
    // Instantly open in same tab for fastest UX on mobile/desktop
    window.location.href = HREF;
  };

  return (
    <button onClick={handleClick} aria-label={label} className={`inline-flex items-center gap-2 ${sizes} ${styles}`}>
