  ...
  return (
    <div className="mt-8">
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((b)=> (
          <article key={b.id} className="card p-6 flex flex-col">
            <h3 className="text-xl font-semibold">{b.title}</h3>
            {b.paragraphs.map((p,idx)=> (
              <p key={idx} className="mt-3 text-sm text-slate-700 leading-7">{p}</p>
            ))}
      ...
      {count < BLOGS.length && (
        <div className="mt-8 flex justify-center">
          <button onClick={()=>setCount(c=>c+6)} className="btn btn-outline">Load more</button>
        </div>
      )}
    </div>
  );
